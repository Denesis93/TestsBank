...
class TestDeposit:
    ...
    def test_deposit(
        self, user_data, request_spec_admin, create_user, request_spec_user, create_account
    ):

        # тело для запроса на пополнение счёта
        # Сумма пополнения (deposit) - минимально 1000, максимально - 9000

        payload_deposit = DepositRequestModel(accountId=create_account.id, amount=AMOUNT_DEPOSIT)
    ...
    def test_deposit_invalid(
        ...
        # тело для запроса на пополнение счёта
        # Сумма пополнения (deposit) - минимально 1000, максимально - 9000
        payload_deposit = DepositRequestModel(accountId=create_account.id, amount=amount)
